...
//Declaration of thread condition varaible
...
//done is global varaible which decide, which waiting thread should be scheduled
int done = 1;
...
void *foo(void *n)
{
	while(1) {
		...
		if (done != (int)*(int *)n) {
		    //value of done and n is not equal , hold wait lock on condition varaible
		    if ((int)*(int *)n == 1) {
